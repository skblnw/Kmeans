...
if __name__ == '__main__':
    ...
    for jj, index_list in enumerate(sorted_unique_initial_list):
        atm_wcss = F[:,index_list]
        ...
        for ii in range(999):
            ...
            #print(atm_Fvalue)
        filename = "AWCSS_cg" + str(ncg) + "_init" + str(jj+1) + "_iter.dat"
        np.savetxt(filename, wcss_iteration_a, fmt="%1.6e")
        filename = "BWCSS_cg" + str(ncg) + "_init" + str(jj+1) + "_iter.dat"
        np.savetxt(filename, wcss_iteration_b, fmt="%1.6e")
        filename = "FWCSS_cg" + str(ncg) + "_init" + str(jj+1) + "_iter.dat"
